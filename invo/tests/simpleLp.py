...
if __name__ == "__main__":
    ...
    print('\n *** Testing ADG with feasible points, hull method norm=1 *** \n')
    interiorPoints = [
    ...
    print(
        '\n *** Testing ADG with feasible points, hull method norm=inf *** \n')
    interiorPoints = [
        np.array([0.1, 0.1, 0.1, 0.1]),
        np.array([0.2, 0.4, 0.2, 0.3]),
        np.array([0.1, 0.3, 0.3, 0.5]),
    ]
    model = AbsoluteDualityGap(normalize_c=np.inf)
    model.FOP(A, b)
    result = simpleLp(vertices, interiorPoints, model)
    for key, value in result.items():
        print('{} = {}'.format(key, value))
    ...
    print(
        '\n *** Testing ADG with feasible and infeasible points, hull method norm=1 *** \n'
    )
    mixedPoints = [
    ...
    print(
        '\n *** Testing ADG with feasible and infeasible points, hull method norm=inf *** \n'
    )
    mixedPoints = [
        np.array([1.1, 0.1, 0.1, 0.1]),
        np.array([0.2, 0.4, 0.2, 0.3]),
    ...
    for key, value in result.items():
        print('{} = {}'.format(key, value))
    projections = model.optimal_points(mixedPoints)
    print(projections)
    # pu.db
    rho = model.rho(mixedPoints)
    print('rho = {}'.format(rho))
